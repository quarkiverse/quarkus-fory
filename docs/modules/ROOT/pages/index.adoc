...
Clients receive standard JSON output consumable by browsers, CLI tools, and any JSON-compliant client.

Fory JSON support is disabled by default, because it takes over the `application/json` media type and would
conflict with other JSON extensions such as Jackson or JSON-B. Enable it explicitly with:
