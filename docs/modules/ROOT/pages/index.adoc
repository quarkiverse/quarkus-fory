...
NOTE: In native mode, types that are not registered through `@ForySerialization` or `@JsonType` fall back to
runtime codec generation, which GraalVM forbids, and requests for them fail with
...
`@JsonType` is optional. Without it, Fory falls back to the interpreted `ObjectCodecBuilder` at runtime, which
works on both the JVM and GraalVM Native Image with Fory 1.7.0 and later.

Annotating a model with `@JsonType` is useful when you want Fory to generate a dedicated codec ahead of
time. For native builds, `ForyJsonGraalVMFeature` is registered through `fory-json`'s
`native-image.properties` and discovers `@JsonType` classes directly during image generation — no extra
wiring is required on the Quarkus side. Choose this path to avoid the interpreted codec's startup cost:
...
JSON codec at compile time. Add the processor to the module that compiles the model:
...
For native builds, disable annotation processing in the native Maven profile. `ForyJsonGraalVMFeature`
discovers `@JsonType` classes directly during image generation, so the processor-generated JSON registry is
not needed — and including it can cause native analysis to fail. Keep the annotation processor enabled for
JVM builds.
